...
try {
    ...
    // Jika kelas_id tidak ada, ambil dari materi yang diampu guru
    if ($kelas_id <= 0) {
        try {
    ...
    if ($kelas_id <= 0) {
        die('Kelas tidak ditemukan atau Anda tidak mengampu materi ini');
    }
    ...
    try {
        ...
        if (!$validate_data) {
            die('Anda tidak memiliki akses untuk mengunduh template materi ini di kelas ini');
        }
    } catch (Exception $e) {
        die('Error validasi: ' . $e->getMessage());
    }
    
    // Ambil nama kelas
    $nama_kelas = '';
    try {
        $stmt_kelas = $conn->prepare("SELECT nama_kelas FROM kelas WHERE id = ? LIMIT 1");
