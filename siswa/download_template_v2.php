<?php
// Versi sederhana untuk test - pastikan nama kelas selalu muncul
...
$kelas_nama_file = 'Semua_Kelas';

// Jika kelas_id > 0, AMBIL NAMA KELAS
if ($kelas_id > 0) {
    // Query langsung untuk memastikan dapat data
    $result = $conn->query("SELECT nama_kelas FROM kelas WHERE id = $kelas_id LIMIT 1");
    ...
    if ($result && $result->num_rows > 0) {
        ...
        $kelas_nama_file = trim($kelas_nama);
        ...
        if (empty($kelas_nama_file)) {
            $kelas_nama_file = 'Kelas_' . $kelas_id;
        }
    } else {
...
if ($kelas_id > 0) {
    $filename .= '_' . $kelas_nama_file; // NAMA KELAS DI BELAKANG
} else {
    $filename .= '_Semua_Kelas';
}
$filename .= '.xlsx';

// Simpan debug info
@file_put_contents('../uploads/temp/debug.txt', 
    "Kelas ID: $kelas_id\n" .
    "Kelas Nama: $kelas_nama\n" .
    "Kelas Nama File: $kelas_nama_file\n" .
    "Filename: $filename\n"
);
